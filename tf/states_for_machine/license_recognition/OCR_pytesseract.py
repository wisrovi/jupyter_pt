...
class OCR_pytesseract(State):
    ...
    def execute(self, **kwargs: dict) -> dict:
        ...
        @param example: a example for input data in file "example_input_kwargs_OCR_pytesseract.json"

        @rtype: dict
        @returns: dict with data processed, add OCR in licenses into frame
        @return example: a example for output data in file "example_output_kwargs_OCR_pytesseract.json"
        """
            ...
            if crop.get("licenses"):
                ...
                for j, license in enumerate(crop.get("licenses")):
                    ...
                    for i in range(len(lines)):
                        if lines[i].strip():
                            line_conf.append((slugify(lines[i]), round(confs[i],3)))
